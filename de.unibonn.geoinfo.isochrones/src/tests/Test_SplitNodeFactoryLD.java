...
public class Test_SplitNodeFactoryLD {
	...
	public static void main(String[] args) throws FileNotFoundException, XMLStreamException {
		...
		for (String layer : layers) {
			...
			var overlayNode = overlay.getDiGraphNode(splitNode.getNodeData());
			var incOverlay = overlayNode.getIncomingArcs().stream()
					.filter(x -> x.getSource().getNodeData().getColor() == Colored.REACHABLE)
					.collect(Collectors.toList());
			var outOverlay = overlayNode.getOutgoingArcs().stream()
					.filter(x -> x.getTarget().getNodeData().getColor() == Colored.UNREACHABLE)
					.collect(Collectors.toList());

			TreeSet<LineSegment> compareSet = LineSegment.compareSet(null, splitNode.getOutgoingArcs().get(0),
					incOverlay, outOverlay);

			int direction = LineSegment.exitDirection(compareSet);
